...
$p_language_delete = NULL; // Initialize page object first

class cp_language_delete extends c_language {
	...
	// Page object name
	var $PageObjName = 'p_language_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("_languagelist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in _language class, _languageinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("_languagelist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($p_language_delete)) $p_language_delete = new cp_language_delete();

// Page init
$p_language_delete->Page_Init();

// Page main
$p_language_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$p_language_delete->Page_Render();
?>
...
var CurrentForm = f_languagedelete = new ew_Form("f_languagedelete", "delete");

// Form_CustomValidate event
f_languagedelete.Form_CustomValidate = 
...
f_languagedelete.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php $p_language_delete->ShowPageHeader(); ?>
<?php
$p_language_delete->ShowMessage();
?>
<form name="f_languagedelete" id="f_languagedelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($p_language_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $p_language_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="_language">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($p_language_delete->RecKeys as $key) { ?>
...
<?php if ($_language->phrase_id->Visible) { // phrase_id ?>
		<th class="<?php echo $_language->phrase_id->HeaderCellClass() ?>"><span id="elh__language_phrase_id" class="_language_phrase_id"><?php echo $_language->phrase_id->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$p_language_delete->RecCnt = 0;
$i = 0;
while (!$p_language_delete->Recordset->EOF) {
	$p_language_delete->RecCnt++;
	$p_language_delete->RowCnt++;

	// Set row properties
	$_language->ResetAttrs();
	$_language->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$p_language_delete->LoadRowValues($p_language_delete->Recordset);

	// Render row
	$p_language_delete->RenderRow();
?>
	<tr<?php echo $_language->RowAttributes() ?>>
<?php if ($_language->phrase_id->Visible) { // phrase_id ?>
		<td<?php echo $_language->phrase_id->CellAttributes() ?>>
<span id="el<?php echo $p_language_delete->RowCnt ?>__language_phrase_id" class="_language_phrase_id">
<span<?php echo $_language->phrase_id->ViewAttributes() ?>>
<?php echo $_language->phrase_id->ListViewValue() ?></span>
</span>
</td>
<?php } ?>
	</tr>
<?php
	$p_language_delete->Recordset->MoveNext();
}
$p_language_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $p_language_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
f_languagedelete.Init();
</script>
<?php
$p_language_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$p_language_delete->Page_Terminate();
?>
