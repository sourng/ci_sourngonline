...
$p_language_edit = NULL; // Initialize page object first

class cp_language_edit extends c_language {
	...
	// Page object name
	var $PageObjName = 'p_language_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			...
			// Load key from Form
			if ($objForm->HasValue("x_phrase_id")) {
				$this->phrase_id->setFormValue($objForm->GetValue("x_phrase_id"));
			}
		} else {
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->phrase_id->FldIsDetailKey)
			$this->phrase_id->setFormValue($objForm->GetValue("x_phrase_id"));
	...
	function RestoreFormValues() {
		global $objForm;
		$this->phrase_id->CurrentValue = $this->phrase_id->FormValue;
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// phrase
			$this->phrase->SetDbValueDef($rsnew, $this->phrase->CurrentValue, "", $this->phrase->ReadOnly);

			// english
			$this->english->SetDbValueDef($rsnew, $this->english->CurrentValue, "", $this->english->ReadOnly);

			// bengali
			$this->bengali->SetDbValueDef($rsnew, $this->bengali->CurrentValue, "", $this->bengali->ReadOnly);

			// spanish
			$this->spanish->SetDbValueDef($rsnew, $this->spanish->CurrentValue, "", $this->spanish->ReadOnly);

			// arabic
			$this->arabic->SetDbValueDef($rsnew, $this->arabic->CurrentValue, "", $this->arabic->ReadOnly);

			// dutch
			$this->dutch->SetDbValueDef($rsnew, $this->dutch->CurrentValue, "", $this->dutch->ReadOnly);

			// russian
			$this->russian->SetDbValueDef($rsnew, $this->russian->CurrentValue, "", $this->russian->ReadOnly);

			// chinese
			$this->chinese->SetDbValueDef($rsnew, $this->chinese->CurrentValue, "", $this->chinese->ReadOnly);

			// turkish
			$this->turkish->SetDbValueDef($rsnew, $this->turkish->CurrentValue, "", $this->turkish->ReadOnly);

			// portuguese
			$this->portuguese->SetDbValueDef($rsnew, $this->portuguese->CurrentValue, "", $this->portuguese->ReadOnly);

			// hungarian
			$this->hungarian->SetDbValueDef($rsnew, $this->hungarian->CurrentValue, "", $this->hungarian->ReadOnly);

			// french
			$this->french->SetDbValueDef($rsnew, $this->french->CurrentValue, "", $this->french->ReadOnly);

			// greek
			$this->greek->SetDbValueDef($rsnew, $this->greek->CurrentValue, "", $this->greek->ReadOnly);

			// german
			$this->german->SetDbValueDef($rsnew, $this->german->CurrentValue, "", $this->german->ReadOnly);

			// italian
			$this->italian->SetDbValueDef($rsnew, $this->italian->CurrentValue, "", $this->italian->ReadOnly);

			// thai
			$this->thai->SetDbValueDef($rsnew, $this->thai->CurrentValue, "", $this->thai->ReadOnly);

			// urdu
			$this->urdu->SetDbValueDef($rsnew, $this->urdu->CurrentValue, "", $this->urdu->ReadOnly);

			// hindi
			$this->hindi->SetDbValueDef($rsnew, $this->hindi->CurrentValue, "", $this->hindi->ReadOnly);

			// latin
			$this->latin->SetDbValueDef($rsnew, $this->latin->CurrentValue, "", $this->latin->ReadOnly);

			// indonesian
			$this->indonesian->SetDbValueDef($rsnew, $this->indonesian->CurrentValue, "", $this->indonesian->ReadOnly);

			// japanese
			$this->japanese->SetDbValueDef($rsnew, $this->japanese->CurrentValue, "", $this->japanese->ReadOnly);

			// korean
			$this->korean->SetDbValueDef($rsnew, $this->korean->CurrentValue, "", $this->korean->ReadOnly);

			// ខ្មែរ
			$this->_178117D2179817C2179A->SetDbValueDef($rsnew, $this->_178117D2179817C2179A->CurrentValue, NULL, $this->_178117D2179817C2179A->ReadOnly);
...
// Create page object
if (!isset($p_language_edit)) $p_language_edit = new cp_language_edit();

// Page init
$p_language_edit->Page_Init();

// Page main
$p_language_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$p_language_edit->Page_Render();
?>
...
var CurrentForm = f_languageedit = new ew_Form("f_languageedit", "edit");

// Validate form
f_languageedit.Validate = function() {
...
f_languageedit.Form_CustomValidate = 
...
f_languageedit.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php $p_language_edit->ShowPageHeader(); ?>
<?php
$p_language_edit->ShowMessage();
?>
<form name="f_languageedit" id="f_languageedit" class="<?php echo $p_language_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($p_language_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $p_language_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="_language">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<input type="hidden" name="modal" value="<?php echo intval($p_language_edit->IsModal) ?>">
<div class="ewEditDiv"><!-- page* -->
<?php if ($_language->phrase_id->Visible) { // phrase_id ?>
	<div id="r_phrase_id" class="form-group">
		<label id="elh__language_phrase_id" class="<?php echo $p_language_edit->LeftColumnClass ?>"><?php echo $_language->phrase_id->FldCaption() ?></label>
		<div class="<?php echo $p_language_edit->RightColumnClass ?>"><div<?php echo $_language->phrase_id->CellAttributes() ?>>
<span id="el__language_phrase_id">
<span<?php echo $_language->phrase_id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $_language->phrase_id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="_language" data-field="x_phrase_id" name="x_phrase_id" id="x_phrase_id" value="<?php echo ew_HtmlEncode($_language->phrase_id->CurrentValue) ?>">
<?php echo $_language->phrase_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($_language->phrase->Visible) { // phrase ?>
	<div id="r_phrase" class="form-group">
		<label id="elh__language_phrase" for="x_phrase" class="<?php echo $p_language_edit->LeftColumnClass ?>"><?php echo $_language->phrase->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $p_language_edit->RightColumnClass ?>"><div<?php echo $_language->phrase->CellAttributes() ?>>
...
<?php if ($_language->english->Visible) { // english ?>
	<div id="r_english" class="form-group">
		<label id="elh__language_english" for="x_english" class="<?php echo $p_language_edit->LeftColumnClass ?>"><?php echo $_language->english->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $p_language_edit->RightColumnClass ?>"><div<?php echo $_language->english->CellAttributes() ?>>
...
<?php if ($_language->bengali->Visible) { // bengali ?>
	<div id="r_bengali" class="form-group">
		<label id="elh__language_bengali" for="x_bengali" class="<?php echo $p_language_edit->LeftColumnClass ?>"><?php echo $_language->bengali->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $p_language_edit->RightColumnClass ?>"><div<?php echo $_language->bengali->CellAttributes() ?>>
...
<?php if ($_language->spanish->Visible) { // spanish ?>
	<div id="r_spanish" class="form-group">
		<label id="elh__language_spanish" for="x_spanish" class="<?php echo $p_language_edit->LeftColumnClass ?>"><?php echo $_language->spanish->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $p_language_edit->RightColumnClass ?>"><div<?php echo $_language->spanish->CellAttributes() ?>>
...
<?php if ($_language->arabic->Visible) { // arabic ?>
	<div id="r_arabic" class="form-group">
		<label id="elh__language_arabic" for="x_arabic" class="<?php echo $p_language_edit->LeftColumnClass ?>"><?php echo $_language->arabic->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $p_language_edit->RightColumnClass ?>"><div<?php echo $_language->arabic->CellAttributes() ?>>
...
<?php if ($_language->dutch->Visible) { // dutch ?>
	<div id="r_dutch" class="form-group">
		<label id="elh__language_dutch" for="x_dutch" class="<?php echo $p_language_edit->LeftColumnClass ?>"><?php echo $_language->dutch->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $p_language_edit->RightColumnClass ?>"><div<?php echo $_language->dutch->CellAttributes() ?>>
...
<?php if ($_language->russian->Visible) { // russian ?>
	<div id="r_russian" class="form-group">
		<label id="elh__language_russian" for="x_russian" class="<?php echo $p_language_edit->LeftColumnClass ?>"><?php echo $_language->russian->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $p_language_edit->RightColumnClass ?>"><div<?php echo $_language->russian->CellAttributes() ?>>
...
<?php if ($_language->chinese->Visible) { // chinese ?>
	<div id="r_chinese" class="form-group">
		<label id="elh__language_chinese" for="x_chinese" class="<?php echo $p_language_edit->LeftColumnClass ?>"><?php echo $_language->chinese->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $p_language_edit->RightColumnClass ?>"><div<?php echo $_language->chinese->CellAttributes() ?>>
...
<?php if ($_language->turkish->Visible) { // turkish ?>
	<div id="r_turkish" class="form-group">
		<label id="elh__language_turkish" for="x_turkish" class="<?php echo $p_language_edit->LeftColumnClass ?>"><?php echo $_language->turkish->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $p_language_edit->RightColumnClass ?>"><div<?php echo $_language->turkish->CellAttributes() ?>>
...
<?php if ($_language->portuguese->Visible) { // portuguese ?>
	<div id="r_portuguese" class="form-group">
		<label id="elh__language_portuguese" for="x_portuguese" class="<?php echo $p_language_edit->LeftColumnClass ?>"><?php echo $_language->portuguese->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $p_language_edit->RightColumnClass ?>"><div<?php echo $_language->portuguese->CellAttributes() ?>>
...
<?php if ($_language->hungarian->Visible) { // hungarian ?>
	<div id="r_hungarian" class="form-group">
		<label id="elh__language_hungarian" for="x_hungarian" class="<?php echo $p_language_edit->LeftColumnClass ?>"><?php echo $_language->hungarian->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $p_language_edit->RightColumnClass ?>"><div<?php echo $_language->hungarian->CellAttributes() ?>>
...
<?php if ($_language->french->Visible) { // french ?>
	<div id="r_french" class="form-group">
		<label id="elh__language_french" for="x_french" class="<?php echo $p_language_edit->LeftColumnClass ?>"><?php echo $_language->french->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $p_language_edit->RightColumnClass ?>"><div<?php echo $_language->french->CellAttributes() ?>>
...
<?php if ($_language->greek->Visible) { // greek ?>
	<div id="r_greek" class="form-group">
		<label id="elh__language_greek" for="x_greek" class="<?php echo $p_language_edit->LeftColumnClass ?>"><?php echo $_language->greek->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $p_language_edit->RightColumnClass ?>"><div<?php echo $_language->greek->CellAttributes() ?>>
...
<?php if ($_language->german->Visible) { // german ?>
	<div id="r_german" class="form-group">
		<label id="elh__language_german" for="x_german" class="<?php echo $p_language_edit->LeftColumnClass ?>"><?php echo $_language->german->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $p_language_edit->RightColumnClass ?>"><div<?php echo $_language->german->CellAttributes() ?>>
...
<?php if ($_language->italian->Visible) { // italian ?>
	<div id="r_italian" class="form-group">
		<label id="elh__language_italian" for="x_italian" class="<?php echo $p_language_edit->LeftColumnClass ?>"><?php echo $_language->italian->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $p_language_edit->RightColumnClass ?>"><div<?php echo $_language->italian->CellAttributes() ?>>
...
<?php if ($_language->thai->Visible) { // thai ?>
	<div id="r_thai" class="form-group">
		<label id="elh__language_thai" for="x_thai" class="<?php echo $p_language_edit->LeftColumnClass ?>"><?php echo $_language->thai->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $p_language_edit->RightColumnClass ?>"><div<?php echo $_language->thai->CellAttributes() ?>>
...
<?php if ($_language->urdu->Visible) { // urdu ?>
	<div id="r_urdu" class="form-group">
		<label id="elh__language_urdu" for="x_urdu" class="<?php echo $p_language_edit->LeftColumnClass ?>"><?php echo $_language->urdu->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $p_language_edit->RightColumnClass ?>"><div<?php echo $_language->urdu->CellAttributes() ?>>
...
<?php if ($_language->hindi->Visible) { // hindi ?>
	<div id="r_hindi" class="form-group">
		<label id="elh__language_hindi" for="x_hindi" class="<?php echo $p_language_edit->LeftColumnClass ?>"><?php echo $_language->hindi->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $p_language_edit->RightColumnClass ?>"><div<?php echo $_language->hindi->CellAttributes() ?>>
...
<?php if ($_language->latin->Visible) { // latin ?>
	<div id="r_latin" class="form-group">
		<label id="elh__language_latin" for="x_latin" class="<?php echo $p_language_edit->LeftColumnClass ?>"><?php echo $_language->latin->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $p_language_edit->RightColumnClass ?>"><div<?php echo $_language->latin->CellAttributes() ?>>
...
<?php if ($_language->indonesian->Visible) { // indonesian ?>
	<div id="r_indonesian" class="form-group">
		<label id="elh__language_indonesian" for="x_indonesian" class="<?php echo $p_language_edit->LeftColumnClass ?>"><?php echo $_language->indonesian->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $p_language_edit->RightColumnClass ?>"><div<?php echo $_language->indonesian->CellAttributes() ?>>
...
<?php if ($_language->japanese->Visible) { // japanese ?>
	<div id="r_japanese" class="form-group">
		<label id="elh__language_japanese" for="x_japanese" class="<?php echo $p_language_edit->LeftColumnClass ?>"><?php echo $_language->japanese->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $p_language_edit->RightColumnClass ?>"><div<?php echo $_language->japanese->CellAttributes() ?>>
...
<?php if ($_language->korean->Visible) { // korean ?>
	<div id="r_korean" class="form-group">
		<label id="elh__language_korean" for="x_korean" class="<?php echo $p_language_edit->LeftColumnClass ?>"><?php echo $_language->korean->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $p_language_edit->RightColumnClass ?>"><div<?php echo $_language->korean->CellAttributes() ?>>
...
<?php if ($_language->_178117D2179817C2179A->Visible) { // ខ្មែរ ?>
	<div id="r__178117D2179817C2179A" class="form-group">
		<label id="elh__language__178117D2179817C2179A" for="x__178117D2179817C2179A" class="<?php echo $p_language_edit->LeftColumnClass ?>"><?php echo $_language->_178117D2179817C2179A->FldCaption() ?></label>
		<div class="<?php echo $p_language_edit->RightColumnClass ?>"><div<?php echo $_language->_178117D2179817C2179A->CellAttributes() ?>>
...
<?php if ($_language->Khmer->Visible) { // Khmer ?>
	<div id="r_Khmer" class="form-group">
		<label id="elh__language_Khmer" for="x_Khmer" class="<?php echo $p_language_edit->LeftColumnClass ?>"><?php echo $_language->Khmer->FldCaption() ?></label>
		<div class="<?php echo $p_language_edit->RightColumnClass ?>"><div<?php echo $_language->Khmer->CellAttributes() ?>>
...
<?php if (!$p_language_edit->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $p_language_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $p_language_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
...
<?php
$p_language_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$p_language_edit->Page_Terminate();
?>
