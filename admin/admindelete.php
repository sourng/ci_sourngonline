...
$admin_delete = NULL; // Initialize page object first

class cadmin_delete extends cadmin {
	...
	function Page_Main() {
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("adminlist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($admin_delete)) $admin_delete = new cadmin_delete();
...
var CurrentForm = fadmindelete = new ew_Form("fadmindelete", "delete");

// Form_CustomValidate event
fadmindelete.Form_CustomValidate = 
...
fadmindelete.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
?>
<form name="fadmindelete" id="fadmindelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($admin_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $admin_delete->Token ?>">
<?php } ?>
...
<?php if ($admin->admin_id->Visible) { // admin_id ?>
		<th class="<?php echo $admin->admin_id->HeaderCellClass() ?>"><span id="elh_admin_admin_id" class="admin_admin_id"><?php echo $admin->admin_id->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
...
<?php if ($admin->admin_id->Visible) { // admin_id ?>
		<td<?php echo $admin->admin_id->CellAttributes() ?>>
<span id="el<?php echo $admin_delete->RowCnt ?>_admin_admin_id" class="admin_admin_id">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $admin_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fadmindelete.Init();
