...
class cadmin extends cTable {
	...
	function __construct() {
		...
		// admin_id
		$this->admin_id = new cField('admin', 'admin', 'x_admin_id', 'admin_id', '`admin_id`', '`admin_id`', 3, -1, FALSE, '`admin_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// name
		$this->name = new cField('admin', 'admin', 'x_name', 'name', '`name`', '`name`', 201, -1, FALSE, '`name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->name->Sortable = TRUE; // Allow sort
		$this->fields['name'] = &$this->name;

		// email
		$this->_email = new cField('admin', 'admin', 'x__email', 'email', '`email`', '`email`', 201, -1, FALSE, '`email`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->_email->Sortable = TRUE; // Allow sort
		$this->fields['email'] = &$this->_email;

		// password
		$this->password = new cField('admin', 'admin', 'x_password', 'password', '`password`', '`password`', 201, -1, FALSE, '`password`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->password->Sortable = TRUE; // Allow sort
		$this->fields['password'] = &$this->password;

		// level
		$this->level = new cField('admin', 'admin', 'x_level', 'level', '`level`', '`level`', 201, -1, FALSE, '`level`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->level->Sortable = TRUE; // Allow sort
		$this->fields['level'] = &$this->level;

		// authentication_key
		$this->authentication_key = new cField('admin', 'admin', 'x_authentication_key', 'authentication_key', '`authentication_key`', '`authentication_key`', 201, -1, FALSE, '`authentication_key`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$rs['admin_id'] = $this->admin_id->DbValue;
		}
		return $bInsert;
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["admin_id"]))
				$arKeys[] = $_POST["admin_id"];
			elseif (isset($_GET["admin_id"]))
				$arKeys[] = $_GET["admin_id"];
			else
