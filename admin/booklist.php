...
class cbook_list extends cbook {
	...
	function RenderListOptions() {
		...
		if ($Security->IsLoggedIn()) {
			if (ew_IsMobile())
				...
				$oListOpt->Body = "<a class=\"ewRowLink ewEdit\" title=\"" . $editcaption . "\" data-table=\"book\" data-caption=\"" . $editcaption . "\" href=\"javascript:void(0);\" onclick=\"ew_ModalDialogShow({lnk:this,btn:'SaveBtn',url:'" . ew_HtmlEncode($this->EditUrl) . "'});\">" . $Language->Phrase("EditLink") . "</a>";
		} else {
			$oListOpt->Body = "";
		}
		...
		if ($Security->IsLoggedIn()) {
			if (ew_IsMobile())
				...
				$oListOpt->Body = "<a class=\"ewRowLink ewCopy\" title=\"" . $copycaption . "\" data-table=\"book\" data-caption=\"" . $copycaption . "\" href=\"javascript:void(0);\" onclick=\"ew_ModalDialogShow({lnk:this,btn:'AddBtn',url:'" . ew_HtmlEncode($this->CopyUrl) . "'});\">" . $Language->Phrase("CopyLink") . "</a>";
		} else {
			$oListOpt->Body = "";
		}
	...
	function SetupOtherOptions() {
		...
		if (ew_IsMobile())
			...
			$item->Body = "<a class=\"ewAddEdit ewAdd\" title=\"" . $addcaption . "\" data-table=\"book\" data-caption=\"" . $addcaption . "\" href=\"javascript:void(0);\" onclick=\"ew_ModalDialogShow({lnk:this,btn:'AddBtn',url:'" . ew_HtmlEncode($this->AddUrl) . "'});\">" . $Language->Phrase("AddLink") . "</a>";
...
fbooklist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php
	...
	if ($bSelectLimit) {
		if ($book_list->TotalRecs <= 0)
			$book_list->TotalRecs = $book->ListRecordCount();
	} else {
...
<div class="box ewBox ewGrid<?php if ($book_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> book">
...
<div id="gmp_book" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($book_list->TotalRecs > 0 || $book->CurrentAction == "gridedit") { ?>
...
<?php if ($book->book_id->Visible) { // book_id ?>
	<?php if ($book->SortUrl($book->book_id) == "") { ?>
		<th data-name="book_id" class="<?php echo $book->book_id->HeaderCellClass() ?>"><div id="elh_book_book_id" class="book_book_id"><div class="ewTableHeaderCaption"><?php echo $book->book_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="book_id" class="<?php echo $book->book_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $book->SortUrl($book->book_id) ?>',1);"><div id="elh_book_book_id" class="book_book_id">
...
<?php if (!isset($book_list->Pager)) $book_list->Pager = new cPrevNextPager($book_list->StartRec, $book_list->DisplayRecs, $book_list->TotalRecs, $book_list->AutoHidePager) ?>
...
<?php } ?>
<?php if ($book_list->Pager->RecordCount > 0) { ?>
