...
$class_grid = NULL; // Initialize page object first

class cclass_grid extends cclass {
	...
	// Page object name
	var $PageObjName = 'class_grid';

	// Grid form hidden field names
	var $FormName = 'fclassgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["class"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->class_id->CurrentValue;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->class_id->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		if (!$this->class_id->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->class_id->CurrentValue = strval($arKeys[0]); // class_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		// Set up foreign key field value from Session
			if ($this->getCurrentMasterTable() == "subject") {
				$this->class_id->CurrentValue = $this->class_id->getSessionValue();
			}
		$conn = &$this->Connection();
