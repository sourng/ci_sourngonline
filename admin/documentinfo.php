...
class cdocument extends cTable {
	...
	function __construct() {
		...
		// document_id
		$this->document_id = new cField('document', 'document', 'x_document_id', 'document_id', '`document_id`', '`document_id`', 3, -1, FALSE, '`document_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->document_id->Sortable = TRUE; // Allow sort
		$this->document_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['document_id'] = &$this->document_id;

		// title
		$this->title = new cField('document', 'document', 'x_title', 'title', '`title`', '`title`', 201, -1, FALSE, '`title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->title->Sortable = TRUE; // Allow sort
		$this->fields['title'] = &$this->title;

		// description
		$this->description = new cField('document', 'document', 'x_description', 'description', '`description`', '`description`', 201, -1, FALSE, '`description`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->description->Sortable = TRUE; // Allow sort
		$this->fields['description'] = &$this->description;

		// file_name
		$this->file_name = new cField('document', 'document', 'x_file_name', 'file_name', '`file_name`', '`file_name`', 201, -1, FALSE, '`file_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->file_name->Sortable = TRUE; // Allow sort
		$this->fields['file_name'] = &$this->file_name;

		// file_type
		$this->file_type = new cField('document', 'document', 'x_file_type', 'file_type', '`file_type`', '`file_type`', 201, -1, FALSE, '`file_type`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->file_type->Sortable = TRUE; // Allow sort
		$this->fields['file_type'] = &$this->file_type;

		// class_id
		$this->class_id = new cField('document', 'document', 'x_class_id', 'class_id', '`class_id`', '`class_id`', 201, -1, FALSE, '`class_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->class_id->Sortable = TRUE; // Allow sort
		$this->fields['class_id'] = &$this->class_id;

		// teacher_id
		$this->teacher_id = new cField('document', 'document', 'x_teacher_id', 'teacher_id', '`teacher_id`', '`teacher_id`', 3, -1, FALSE, '`teacher_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// timestamp
		$this->timestamp = new cField('document', 'document', 'x_timestamp', 'timestamp', '`timestamp`', '`timestamp`', 201, -1, FALSE, '`timestamp`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`document`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->document_id->setDbValue($conn->Insert_ID());
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if ($rs) {
			if (array_key_exists('document_id', $rs))
				ew_AddFilter($where, ew_QuotedName('document_id', $this->DBID) . '=' . ew_QuotedValue($rs['document_id'], $this->document_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->document_id->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->document_id->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@document_id@", ew_AdjustSql($this->document_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "documentedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "documentadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("documentview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("documentview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "documentadd.php?" . $this->UrlParm($parm);
		else
			$url = "documentadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("documentedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("documentadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "document_id:" . ew_VarToJson($this->document_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->document_id->CurrentValue)) {
			$sUrl .= "document_id=" . urlencode($this->document_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["document_id"]))
				$arKeys[] = $_POST["document_id"];
			elseif (isset($_GET["document_id"]))
				$arKeys[] = $_GET["document_id"];
			else
	...
	function LoadListRowValues(&$rs) {
		$this->document_id->setDbValue($rs->fields('document_id'));
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->document_id->Exportable) $Doc->ExportCaption($this->document_id);
					if ($this->title->Exportable) $Doc->ExportCaption($this->title);
					if ($this->description->Exportable) $Doc->ExportCaption($this->description);
					if ($this->file_name->Exportable) $Doc->ExportCaption($this->file_name);
					if ($this->file_type->Exportable) $Doc->ExportCaption($this->file_type);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->document_id->Exportable) $Doc->ExportField($this->document_id);
						if ($this->title->Exportable) $Doc->ExportField($this->title);
						if ($this->description->Exportable) $Doc->ExportField($this->description);
						if ($this->file_name->Exportable) $Doc->ExportField($this->file_name);
						if ($this->file_type->Exportable) $Doc->ExportField($this->file_type);
