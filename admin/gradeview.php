...
$grade_view = NULL; // Initialize page object first

class cgrade_view extends cgrade {
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["grade_id"] <> "") {
			$this->RecKey["grade_id"] = $_GET["grade_id"];
			$KeyUrl .= "&amp;grade_id=" . urlencode($this->RecKey["grade_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["grade_id"] <> "") {
				...
				$this->RecKey["grade_id"] = $this->grade_id->QueryStringValue;
			} elseif (@$_POST["grade_id"] <> "") {
				$this->grade_id->setFormValue($_POST["grade_id"]);
				$this->RecKey["grade_id"] = $this->grade_id->FormValue;
			} else {
				$sReturnUrl = "gradelist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "gradelist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "gradelist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($grade_view)) $grade_view = new cgrade_view();

// Page init
$grade_view->Page_Init();

// Page main
$grade_view->Page_Main();
...
var CurrentForm = fgradeview = new ew_Form("fgradeview", "view");

// Form_CustomValidate event
fgradeview.Form_CustomValidate = 
...
fgradeview.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php
	foreach ($grade_view->OtherOptions as &$option)
		$option->Render("body");
?>
...
?>
<form name="fgradeview" id="fgradeview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($grade_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $grade_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="grade">
<input type="hidden" name="modal" value="<?php echo intval($grade_view->IsModal) ?>">
<table class="table table-striped table-bordered table-hover table-condensed ewViewTable">
<?php if ($grade->grade_id->Visible) { // grade_id ?>
	<tr id="r_grade_id">
		<td class="col-sm-2"><span id="elh_grade_grade_id"><?php echo $grade->grade_id->FldCaption() ?></span></td>
...
<?php echo $grade->grade_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($grade->name->Visible) { // name ?>
	<tr id="r_name">
		<td class="col-sm-2"><span id="elh_grade_name"><?php echo $grade->name->FldCaption() ?></span></td>
		<td data-name="name"<?php echo $grade->name->CellAttributes() ?>>
<span id="el_grade_name">
<span<?php echo $grade->name->ViewAttributes() ?>>
<?php echo $grade->name->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($grade->grade_point->Visible) { // grade_point ?>
	<tr id="r_grade_point">
		<td class="col-sm-2"><span id="elh_grade_grade_point"><?php echo $grade->grade_point->FldCaption() ?></span></td>
		<td data-name="grade_point"<?php echo $grade->grade_point->CellAttributes() ?>>
<span id="el_grade_grade_point">
<span<?php echo $grade->grade_point->ViewAttributes() ?>>
<?php echo $grade->grade_point->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($grade->mark_from->Visible) { // mark_from ?>
	<tr id="r_mark_from">
		<td class="col-sm-2"><span id="elh_grade_mark_from"><?php echo $grade->mark_from->FldCaption() ?></span></td>
...
<?php echo $grade->mark_from->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($grade->mark_upto->Visible) { // mark_upto ?>
	<tr id="r_mark_upto">
		<td class="col-sm-2"><span id="elh_grade_mark_upto"><?php echo $grade->mark_upto->FldCaption() ?></span></td>
...
<?php echo $grade->mark_upto->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($grade->comment->Visible) { // comment ?>
	<tr id="r_comment">
		<td class="col-sm-2"><span id="elh_grade_comment"><?php echo $grade->comment->FldCaption() ?></span></td>
		<td data-name="comment"<?php echo $grade->comment->CellAttributes() ?>>
<span id="el_grade_comment">
<span<?php echo $grade->comment->ViewAttributes() ?>>
<?php echo $grade->comment->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
