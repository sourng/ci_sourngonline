...
class cinvoice extends cTable {
	...
	function __construct() {
		...
		// invoice_id
		$this->invoice_id = new cField('invoice', 'invoice', 'x_invoice_id', 'invoice_id', '`invoice_id`', '`invoice_id`', 3, -1, FALSE, '`invoice_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// student_id
		$this->student_id = new cField('invoice', 'invoice', 'x_student_id', 'student_id', '`student_id`', '`student_id`', 3, -1, FALSE, '`student_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// title
		$this->title = new cField('invoice', 'invoice', 'x_title', 'title', '`title`', '`title`', 201, -1, FALSE, '`title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->title->Sortable = TRUE; // Allow sort
		$this->fields['title'] = &$this->title;

		// description
		$this->description = new cField('invoice', 'invoice', 'x_description', 'description', '`description`', '`description`', 201, -1, FALSE, '`description`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->description->Sortable = TRUE; // Allow sort
		$this->fields['description'] = &$this->description;

		// amount
		$this->amount = new cField('invoice', 'invoice', 'x_amount', 'amount', '`amount`', '`amount`', 3, -1, FALSE, '`amount`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->amount->Sortable = TRUE; // Allow sort
		$this->amount->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['amount'] = &$this->amount;

		// amount_paid
		$this->amount_paid = new cField('invoice', 'invoice', 'x_amount_paid', 'amount_paid', '`amount_paid`', '`amount_paid`', 201, -1, FALSE, '`amount_paid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->amount_paid->Sortable = TRUE; // Allow sort
		$this->fields['amount_paid'] = &$this->amount_paid;

		// due
		$this->due = new cField('invoice', 'invoice', 'x_due', 'due', '`due`', '`due`', 201, -1, FALSE, '`due`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->due->Sortable = TRUE; // Allow sort
		$this->fields['due'] = &$this->due;

		// creation_timestamp
		$this->creation_timestamp = new cField('invoice', 'invoice', 'x_creation_timestamp', 'creation_timestamp', '`creation_timestamp`', '`creation_timestamp`', 3, -1, FALSE, '`creation_timestamp`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->creation_timestamp->Sortable = TRUE; // Allow sort
		$this->creation_timestamp->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['creation_timestamp'] = &$this->creation_timestamp;

		// payment_timestamp
		$this->payment_timestamp = new cField('invoice', 'invoice', 'x_payment_timestamp', 'payment_timestamp', '`payment_timestamp`', '`payment_timestamp`', 201, -1, FALSE, '`payment_timestamp`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->payment_timestamp->Sortable = TRUE; // Allow sort
		$this->fields['payment_timestamp'] = &$this->payment_timestamp;

		// payment_method
		$this->payment_method = new cField('invoice', 'invoice', 'x_payment_method', 'payment_method', '`payment_method`', '`payment_method`', 201, -1, FALSE, '`payment_method`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->payment_method->Sortable = TRUE; // Allow sort
		$this->fields['payment_method'] = &$this->payment_method;

		// payment_details
		$this->payment_details = new cField('invoice', 'invoice', 'x_payment_details', 'payment_details', '`payment_details`', '`payment_details`', 201, -1, FALSE, '`payment_details`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->payment_details->Sortable = TRUE; // Allow sort
		$this->fields['payment_details'] = &$this->payment_details;

		// status
		$this->status = new cField('invoice', 'invoice', 'x_status', 'status', '`status`', '`status`', 201, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`invoice`";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "invoiceedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "invoiceadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("invoiceview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("invoiceview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "invoiceadd.php?" . $this->UrlParm($parm);
		else
			$url = "invoiceadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("invoiceedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("invoiceadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("invoicedelete.php", $this->UrlParm());
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["invoice_id"]))
				$arKeys[] = $_POST["invoice_id"];
			elseif (isset($_GET["invoice_id"]))
				$arKeys[] = $_GET["invoice_id"];
			else
	...
	function LoadListRowValues(&$rs) {
		...
		$this->creation_timestamp->setDbValue($rs->fields('creation_timestamp'));
		$this->payment_timestamp->setDbValue($rs->fields('payment_timestamp'));
	...
	function RenderEditRow() {
		...
		$this->due->EditValue = $this->due->CurrentValue;
		...
		$this->creation_timestamp->EditValue = $this->creation_timestamp->CurrentValue;
		...
		$this->payment_timestamp->EditValue = $this->payment_timestamp->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->due->Exportable) $Doc->ExportCaption($this->due);
					if ($this->creation_timestamp->Exportable) $Doc->ExportCaption($this->creation_timestamp);
					if ($this->payment_timestamp->Exportable) $Doc->ExportCaption($this->payment_timestamp);
					if ($this->payment_method->Exportable) $Doc->ExportCaption($this->payment_method);
					if ($this->payment_details->Exportable) $Doc->ExportCaption($this->payment_details);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->due->Exportable) $Doc->ExportField($this->due);
						if ($this->creation_timestamp->Exportable) $Doc->ExportField($this->creation_timestamp);
						if ($this->payment_timestamp->Exportable) $Doc->ExportField($this->payment_timestamp);
						if ($this->payment_method->Exportable) $Doc->ExportField($this->payment_method);
						if ($this->payment_details->Exportable) $Doc->ExportField($this->payment_details);
