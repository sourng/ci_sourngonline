...
//
// Table class for parent
//
class cparent extends cTable {
	var $parent_id;
	var $name;
	var $_email;
	var $password;
	...
	function __construct() {
		...
		$this->TableVar = 'parent';
		$this->TableName = 'parent';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`parent`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// parent_id
		$this->parent_id = new cField('parent', 'parent', 'x_parent_id', 'parent_id', '`parent_id`', '`parent_id`', 3, -1, FALSE, '`parent_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// name
		$this->name = new cField('parent', 'parent', 'x_name', 'name', '`name`', '`name`', 201, -1, FALSE, '`name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->name->Sortable = TRUE; // Allow sort
		$this->fields['name'] = &$this->name;

		// email
		$this->_email = new cField('parent', 'parent', 'x__email', 'email', '`email`', '`email`', 201, -1, FALSE, '`email`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->_email->Sortable = TRUE; // Allow sort
		$this->fields['email'] = &$this->_email;

		// password
		$this->password = new cField('parent', 'parent', 'x_password', 'password', '`password`', '`password`', 201, -1, FALSE, '`password`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->password->Sortable = TRUE; // Allow sort
		$this->fields['password'] = &$this->password;

		// phone
		$this->phone = new cField('parent', 'parent', 'x_phone', 'phone', '`phone`', '`phone`', 201, -1, FALSE, '`phone`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->phone->Sortable = TRUE; // Allow sort
		$this->fields['phone'] = &$this->phone;

		// address
		$this->address = new cField('parent', 'parent', 'x_address', 'address', '`address`', '`address`', 201, -1, FALSE, '`address`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->address->Sortable = TRUE; // Allow sort
		$this->fields['address'] = &$this->address;

		// profession
		$this->profession = new cField('parent', 'parent', 'x_profession', 'profession', '`profession`', '`profession`', 201, -1, FALSE, '`profession`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->profession->Sortable = TRUE; // Allow sort
		$this->fields['profession'] = &$this->profession;

		// authentication_key
		$this->authentication_key = new cField('parent', 'parent', 'x_authentication_key', 'authentication_key', '`authentication_key`', '`authentication_key`', 201, -1, FALSE, '`authentication_key`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`parent`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->parent_id->setDbValue($conn->Insert_ID());
			$rs['parent_id'] = $this->parent_id->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('parent_id', $rs))
				ew_AddFilter($where, ew_QuotedName('parent_id', $this->DBID) . '=' . ew_QuotedValue($rs['parent_id'], $this->parent_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->parent_id->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->parent_id->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@parent_id@", ew_AdjustSql($this->parent_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "parentedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "parentadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("parentview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("parentview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "parentadd.php?" . $this->UrlParm($parm);
		else
			$url = "parentadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("parentedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("parentadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("parentdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "parent_id:" . ew_VarToJson($this->parent_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->parent_id->CurrentValue)) {
			$sUrl .= "parent_id=" . urlencode($this->parent_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["parent_id"]))
				$arKeys[] = $_POST["parent_id"];
			elseif (isset($_GET["parent_id"]))
				$arKeys[] = $_GET["parent_id"];
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->parent_id->CurrentValue = $key;
