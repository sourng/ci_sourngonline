...
$payment_delete = NULL; // Initialize page object first

class cpayment_delete extends cpayment {
	...
	// Page object name
	var $PageObjName = 'payment_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("paymentlist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in payment class, paymentinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("paymentlist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($payment_delete)) $payment_delete = new cpayment_delete();

// Page init
$payment_delete->Page_Init();

// Page main
$payment_delete->Page_Main();
...
var CurrentForm = fpaymentdelete = new ew_Form("fpaymentdelete", "delete");

// Form_CustomValidate event
fpaymentdelete.Form_CustomValidate = 
...
fpaymentdelete.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
?>
<form name="fpaymentdelete" id="fpaymentdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($payment_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $payment_delete->Token ?>">
<?php } ?>
...
<?php if ($payment->payment_id->Visible) { // payment_id ?>
		<th class="<?php echo $payment->payment_id->HeaderCellClass() ?>"><span id="elh_payment_payment_id" class="payment_payment_id"><?php echo $payment->payment_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($payment->expense_category_id->Visible) { // expense_category_id ?>
		<th class="<?php echo $payment->expense_category_id->HeaderCellClass() ?>"><span id="elh_payment_expense_category_id" class="payment_expense_category_id"><?php echo $payment->expense_category_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($payment->invoice_id->Visible) { // invoice_id ?>
		<th class="<?php echo $payment->invoice_id->HeaderCellClass() ?>"><span id="elh_payment_invoice_id" class="payment_invoice_id"><?php echo $payment->invoice_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($payment->student_id->Visible) { // student_id ?>
		<th class="<?php echo $payment->student_id->HeaderCellClass() ?>"><span id="elh_payment_student_id" class="payment_student_id"><?php echo $payment->student_id->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
...
while (!$payment_delete->Recordset->EOF) {
	...
	// Set row properties
	$payment->ResetAttrs();
	$payment->RowType = EW_ROWTYPE_VIEW; // View
...
<?php if ($payment->payment_id->Visible) { // payment_id ?>
		<td<?php echo $payment->payment_id->CellAttributes() ?>>
<span id="el<?php echo $payment_delete->RowCnt ?>_payment_payment_id" class="payment_payment_id">
...
<?php } ?>
<?php if ($payment->expense_category_id->Visible) { // expense_category_id ?>
		<td<?php echo $payment->expense_category_id->CellAttributes() ?>>
<span id="el<?php echo $payment_delete->RowCnt ?>_payment_expense_category_id" class="payment_expense_category_id">
...
<?php } ?>
<?php if ($payment->invoice_id->Visible) { // invoice_id ?>
		<td<?php echo $payment->invoice_id->CellAttributes() ?>>
<span id="el<?php echo $payment_delete->RowCnt ?>_payment_invoice_id" class="payment_invoice_id">
...
<?php } ?>
<?php if ($payment->student_id->Visible) { // student_id ?>
		<td<?php echo $payment->student_id->CellAttributes() ?>>
<span id="el<?php echo $payment_delete->RowCnt ?>_payment_student_id" class="payment_student_id">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $payment_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fpaymentdelete.Init();
