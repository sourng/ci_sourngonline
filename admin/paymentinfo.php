...
class cpayment extends cTable {
	...
	function __construct() {
		...
		// payment_id
		$this->payment_id = new cField('payment', 'payment', 'x_payment_id', 'payment_id', '`payment_id`', '`payment_id`', 3, -1, FALSE, '`payment_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->payment_id->Sortable = TRUE; // Allow sort
		$this->payment_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['payment_id'] = &$this->payment_id;

		// expense_category_id
		$this->expense_category_id = new cField('payment', 'payment', 'x_expense_category_id', 'expense_category_id', '`expense_category_id`', '`expense_category_id`', 3, -1, FALSE, '`expense_category_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->expense_category_id->Sortable = TRUE; // Allow sort
		...
		// title
		$this->title = new cField('payment', 'payment', 'x_title', 'title', '`title`', '`title`', 201, -1, FALSE, '`title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->title->Sortable = TRUE; // Allow sort
		$this->fields['title'] = &$this->title;

		// payment_type
		$this->payment_type = new cField('payment', 'payment', 'x_payment_type', 'payment_type', '`payment_type`', '`payment_type`', 201, -1, FALSE, '`payment_type`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->payment_type->Sortable = TRUE; // Allow sort
		$this->fields['payment_type'] = &$this->payment_type;

		// invoice_id
		$this->invoice_id = new cField('payment', 'payment', 'x_invoice_id', 'invoice_id', '`invoice_id`', '`invoice_id`', 3, -1, FALSE, '`invoice_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// student_id
		$this->student_id = new cField('payment', 'payment', 'x_student_id', 'student_id', '`student_id`', '`student_id`', 3, -1, FALSE, '`student_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// method
		$this->method = new cField('payment', 'payment', 'x_method', 'method', '`method`', '`method`', 201, -1, FALSE, '`method`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->method->Sortable = TRUE; // Allow sort
		$this->fields['method'] = &$this->method;

		// description
		$this->description = new cField('payment', 'payment', 'x_description', 'description', '`description`', '`description`', 201, -1, FALSE, '`description`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->description->Sortable = TRUE; // Allow sort
		$this->fields['description'] = &$this->description;

		// amount
		$this->amount = new cField('payment', 'payment', 'x_amount', 'amount', '`amount`', '`amount`', 201, -1, FALSE, '`amount`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->amount->Sortable = TRUE; // Allow sort
		$this->fields['amount'] = &$this->amount;

		// timestamp
		$this->timestamp = new cField('payment', 'payment', 'x_timestamp', 'timestamp', '`timestamp`', '`timestamp`', 201, -1, FALSE, '`timestamp`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`payment`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("paymentview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("paymentview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "paymentadd.php?" . $this->UrlParm($parm);
		else
			$url = "paymentadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("paymentedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("paymentadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("paymentdelete.php", $this->UrlParm());
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["payment_id"]))
				$arKeys[] = $_POST["payment_id"];
			elseif (isset($_GET["payment_id"]))
				$arKeys[] = $_GET["payment_id"];
			else
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->expense_category_id->Exportable) $Doc->ExportCaption($this->expense_category_id);
					...
					if ($this->method->Exportable) $Doc->ExportCaption($this->method);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->expense_category_id->Exportable) $Doc->ExportField($this->expense_category_id);
						...
						if ($this->method->Exportable) $Doc->ExportField($this->method);
