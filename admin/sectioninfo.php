...
//
// Table class for section
//
class csection extends cTable {
	var $section_id;
	var $name;
	var $nick_name;
	var $class_id;
	...
	function __construct() {
		...
		// section_id
		$this->section_id = new cField('section', 'section', 'x_section_id', 'section_id', '`section_id`', '`section_id`', 3, -1, FALSE, '`section_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// name
		$this->name = new cField('section', 'section', 'x_name', 'name', '`name`', '`name`', 201, -1, FALSE, '`name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->name->Sortable = TRUE; // Allow sort
		$this->fields['name'] = &$this->name;

		// nick_name
		$this->nick_name = new cField('section', 'section', 'x_nick_name', 'nick_name', '`nick_name`', '`nick_name`', 201, -1, FALSE, '`nick_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->nick_name->Sortable = TRUE; // Allow sort
		$this->fields['nick_name'] = &$this->nick_name;

		// class_id
		$this->class_id = new cField('section', 'section', 'x_class_id', 'class_id', '`class_id`', '`class_id`', 3, -1, FALSE, '`class_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// teacher_id
		$this->teacher_id = new cField('section', 'section', 'x_teacher_id', 'teacher_id', '`teacher_id`', '`teacher_id`', 3, -1, FALSE, '`teacher_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`section`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->section_id->setDbValue($conn->Insert_ID());
			$rs['section_id'] = $this->section_id->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('section_id', $rs))
				ew_AddFilter($where, ew_QuotedName('section_id', $this->DBID) . '=' . ew_QuotedValue($rs['section_id'], $this->section_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->section_id->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->section_id->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@section_id@", ew_AdjustSql($this->section_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "sectionedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "sectionadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("sectionview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("sectionview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "sectionadd.php?" . $this->UrlParm($parm);
		else
			$url = "sectionadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("sectionedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("sectionadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("sectiondelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "section_id:" . ew_VarToJson($this->section_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->section_id->CurrentValue)) {
			$sUrl .= "section_id=" . urlencode($this->section_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["section_id"]))
				$arKeys[] = $_POST["section_id"];
			elseif (isset($_GET["section_id"]))
				$arKeys[] = $_GET["section_id"];
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->section_id->CurrentValue = $key;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->nick_name->Exportable) $Doc->ExportCaption($this->nick_name);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->nick_name->Exportable) $Doc->ExportField($this->nick_name);
