...
class csettings_add extends csettings {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["settings_id"] != "") {
				$this->settings_id->setQueryStringValue($_GET["settings_id"]);
				$this->setKey("settings_id", $this->settings_id->CurrentValue); // Set up key
			} else {
				$this->setKey("settings_id", ""); // Clear key
	...
	function LoadDefaultValues() {
		$this->settings_id->CurrentValue = NULL;
		$this->settings_id->OldValue = $this->settings_id->CurrentValue;
	...
	function NewRow() {
		...
		$row = array();
		$row['settings_id'] = $this->settings_id->CurrentValue;
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $settings->type->FldCaption(), $settings->type->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_description");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $settings->description->FldCaption(), $settings->description->ReqErrMsg)) ?>");
...
fsettingsadd.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<input type="hidden" name="modal" value="<?php echo intval($settings_add->IsModal) ?>">
<div class="ewAddDiv"><!-- page* -->
<?php if ($settings->type->Visible) { // type ?>
	<div id="r_type" class="form-group">
		<label id="elh_settings_type" for="x_type" class="<?php echo $settings_add->LeftColumnClass ?>"><?php echo $settings->type->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $settings_add->RightColumnClass ?>"><div<?php echo $settings->type->CellAttributes() ?>>
<span id="el_settings_type">
<textarea data-table="settings" data-field="x_type" name="x_type" id="x_type" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($settings->type->getPlaceHolder()) ?>"<?php echo $settings->type->EditAttributes() ?>><?php echo $settings->type->EditValue ?></textarea>
</span>
<?php echo $settings->type->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($settings->description->Visible) { // description ?>
	<div id="r_description" class="form-group">
		<label id="elh_settings_description" for="x_description" class="<?php echo $settings_add->LeftColumnClass ?>"><?php echo $settings->description->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $settings_add->RightColumnClass ?>"><div<?php echo $settings->description->CellAttributes() ?>>
<span id="el_settings_description">
<textarea data-table="settings" data-field="x_description" name="x_description" id="x_description" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($settings->description->getPlaceHolder()) ?>"<?php echo $settings->description->EditAttributes() ?>><?php echo $settings->description->EditValue ?></textarea>
</span>
<?php echo $settings->description->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
<?php if (!$settings_add->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $settings_add->OffsetColumnClass ?>"><!-- buttons offset -->
