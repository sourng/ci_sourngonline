...
class csettings_list extends csettings {
	...
	function Page_Init() {
		...
		$this->settings_id->SetVisibility();
		if ($this->IsAdd() || $this->IsCopy() || $this->IsGridAdd())
			$this->settings_id->Visible = FALSE;
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->settings_id->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($this->settings_id->FormValue))
				return FALSE;
		}
		return TRUE;
	}
	...
	function SetupSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$this->UpdateSort($this->settings_id); // settings_id
	...
	function RenderListOptions() {
		...
		if ($Security->IsLoggedIn()) {
			if (ew_IsMobile())
				...
				$oListOpt->Body = "<a class=\"ewRowLink ewEdit\" title=\"" . $editcaption . "\" data-table=\"settings\" data-caption=\"" . $editcaption . "\" href=\"javascript:void(0);\" onclick=\"ew_ModalDialogShow({lnk:this,btn:'SaveBtn',url:'" . ew_HtmlEncode($this->EditUrl) . "'});\">" . $Language->Phrase("EditLink") . "</a>";
		} else {
			$oListOpt->Body = "";
		}
		...
		if ($Security->IsLoggedIn()) {
			if (ew_IsMobile())
				...
				$oListOpt->Body = "<a class=\"ewRowLink ewCopy\" title=\"" . $copycaption . "\" data-table=\"settings\" data-caption=\"" . $copycaption . "\" href=\"javascript:void(0);\" onclick=\"ew_ModalDialogShow({lnk:this,btn:'AddBtn',url:'" . ew_HtmlEncode($this->CopyUrl) . "'});\">" . $Language->Phrase("CopyLink") . "</a>";
		} else {
			$oListOpt->Body = "";
		}
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" class=\"ewMultiSelect\" value=\"" . ew_HtmlEncode($this->settings_id->CurrentValue) . "\" onclick=\"ew_ClickMultiCheckbox(event);\">";
	...
	function SetupOtherOptions() {
		...
		if (ew_IsMobile())
			...
			$item->Body = "<a class=\"ewAddEdit ewAdd\" title=\"" . $addcaption . "\" data-table=\"settings\" data-caption=\"" . $addcaption . "\" href=\"javascript:void(0);\" onclick=\"ew_ModalDialogShow({lnk:this,btn:'AddBtn',url:'" . ew_HtmlEncode($this->AddUrl) . "'});\">" . $Language->Phrase("AddLink") . "</a>";
	...
	function LoadOldRecord() {
		...
		$bValidKey = TRUE;
		if (strval($this->getKey("settings_id")) <> "")
			$this->settings_id->CurrentValue = $this->getKey("settings_id"); // settings_id
		else
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		$this->settings_id->ViewCustomAttributes = "";

			// settings_id
			$this->settings_id->LinkCustomAttributes = "";
...
fsettingslist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php
	...
	if ($bSelectLimit) {
		if ($settings_list->TotalRecs <= 0)
			$settings_list->TotalRecs = $settings->ListRecordCount();
	} else {
...
<div class="box ewBox ewGrid<?php if ($settings_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> settings">
...
<div id="gmp_settings" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
...
?>
<?php if ($settings->settings_id->Visible) { // settings_id ?>
	<?php if ($settings->SortUrl($settings->settings_id) == "") { ?>
		<th data-name="settings_id" class="<?php echo $settings->settings_id->HeaderCellClass() ?>"><div id="elh_settings_settings_id" class="settings_settings_id"><div class="ewTableHeaderCaption"><?php echo $settings->settings_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="settings_id" class="<?php echo $settings->settings_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $settings->SortUrl($settings->settings_id) ?>',1);"><div id="elh_settings_settings_id" class="settings_settings_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $settings->settings_id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($settings->settings_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($settings->settings_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php
...
?>
	<?php if ($settings->settings_id->Visible) { // settings_id ?>
		<td data-name="settings_id"<?php echo $settings->settings_id->CellAttributes() ?>>
<span id="el<?php echo $settings_list->RowCnt ?>_settings_settings_id" class="settings_settings_id">
<span<?php echo $settings->settings_id->ViewAttributes() ?>>
<?php echo $settings->settings_id->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
<?php
...
<?php if (!isset($settings_list->Pager)) $settings_list->Pager = new cPrevNextPager($settings_list->StartRec, $settings_list->DisplayRecs, $settings_list->TotalRecs, $settings_list->AutoHidePager) ?>
