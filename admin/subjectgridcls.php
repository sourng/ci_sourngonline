...
$subject_grid = NULL; // Initialize page object first

class csubject_grid extends csubject {
	...
	// Page object name
	var $PageObjName = 'subject_grid';

	// Grid form hidden field names
	var $FormName = 'fsubjectgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["subject"];

		}
		$this->AddUrl = "subjectadd.php";
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_image") && $objForm->HasValue("o_image") && $this->image->CurrentValue <> $this->image->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_class_id") && $objForm->HasValue("o_class_id") && $this->class_id->CurrentValue <> $this->class_id->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->subject_id->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->class_id->setOldValue($objForm->GetValue("o_class_id"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->subject_id->CurrentValue = strval($arKeys[0]); // subject_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "teacher") {
			...
			if ($GLOBALS["teacher"]->EventCancelled) $this->EventCancelled = TRUE;
		}
