...
class csubject extends cTable {
	...
	function __construct() {
		...
		// name
		$this->name = new cField('subject', 'subject', 'x_name', 'name', '`name`', '`name`', 201, -1, FALSE, '`name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->name->Sortable = TRUE; // Allow sort
		$this->fields['name'] = &$this->name;

		// description
		$this->description = new cField('subject', 'subject', 'x_description', 'description', '`description`', '`description`', 201, -1, FALSE, '`description`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->description->Sortable = TRUE; // Allow sort
		$this->fields['description'] = &$this->description;

		// image
		$this->image = new cField('subject', 'subject', 'x_image', 'image', '`image`', '`image`', 200, -1, FALSE, '`image`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->image->Sortable = TRUE; // Allow sort
		$this->fields['image'] = &$this->image;

		// class_id
		$this->class_id = new cField('subject', 'subject', 'x_class_id', 'class_id', '`class_id`', '`class_id`', 3, -1, FALSE, '`class_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// teacher_id
		$this->teacher_id = new cField('subject', 'subject', 'x_teacher_id', 'teacher_id', '`teacher_id`', '`teacher_id`', 3, -1, FALSE, '`teacher_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function GetMasterFilter() {
		...
		// Master filter
		$sMasterFilter = "";
		if ($this->getCurrentMasterTable() == "teacher") {
			if ($this->teacher_id->getSessionValue() <> "")
				$sMasterFilter .= "`teacher_id`=" . ew_QuotedValue($this->teacher_id->getSessionValue(), EW_DATATYPE_NUMBER, "DB");
			else
				return "";
		}
	...
	// Master filter
	function SqlMasterFilter_teacher() {
		return "`teacher_id`=@teacher_id@";
	}

	// Detail filter
	function SqlDetailFilter_teacher() {
		return "`teacher_id`=@teacher_id@";
	}
	...
	function GetDetailUrl() {
		...
		// Detail url
		$sDetailUrl = "";
		if ($this->getCurrentDetailTable() == "class") {
			$sDetailUrl = $GLOBALS["class"]->GetListUrl() . "?" . EW_TABLE_SHOW_MASTER . "=" . $this->TableVar;
			$sDetailUrl .= "&fk_class_id=" . urlencode($this->class_id->CurrentValue);
		}
		if ($sDetailUrl == "") {
	...
	function AddMasterUrl($url) {
		if ($this->getCurrentMasterTable() == "teacher" && strpos($url, EW_TABLE_SHOW_MASTER . "=") === FALSE) {
			$url .= (strpos($url, "?") !== FALSE ? "&" : "?") . EW_TABLE_SHOW_MASTER . "=" . $this->getCurrentMasterTable();
			$url .= "&fk_teacher_id=" . urlencode($this->teacher_id->CurrentValue);
		}
		return $url;
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["subject_id"]))
				$arKeys[] = $_POST["subject_id"];
			elseif (isset($_GET["subject_id"]))
				$arKeys[] = $_GET["subject_id"];
			else
