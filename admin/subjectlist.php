...
class csubject_list extends csubject {
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "teacher") {
			global $teacher;
			$rsmaster = $teacher->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("teacherlist.php"); // Return to master page
			} else {
				$teacher->LoadListRowValues($rsmaster);
				$teacher->RowType = EW_ROWTYPE_MASTER; // Master row
				$teacher->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function SetupListOptions() {
		...
		// "detail_class"
		$item = &$this->ListOptions->Add("detail_class");
		$item->CssClass = "text-nowrap";
	...
	function RenderListOptions() {
		...
		// "detail_class"
		$oListOpt = &$this->ListOptions->Items["detail_class"];
		if ($Security->IsLoggedIn()) {
			$body = $Language->Phrase("DetailLink") . $Language->TablePhrase("class", "TblCaption");
			$body = "<a class=\"btn btn-default btn-sm ewRowLink ewDetail\" data-action=\"list\" href=\"" . ew_HtmlEncode("classlist.php?" . EW_TABLE_SHOW_MASTER . "=subject&fk_class_id=" . urlencode(strval($this->class_id->CurrentValue)) . "") . "\">" . $body . "</a>";
			$links = "";
			if ($GLOBALS["class_grid"]->DetailView && $Security->IsLoggedIn() && $Security->IsLoggedIn()) {
				$caption = $Language->Phrase("MasterDetailViewLink");
				$url = $this->GetViewUrl(EW_TABLE_SHOW_DETAIL . "=class");
				...
				$DetailViewTblVar .= "class";
			}
			if ($GLOBALS["class_grid"]->DetailEdit && $Security->IsLoggedIn() && $Security->IsLoggedIn()) {
				$caption = $Language->Phrase("MasterDetailEditLink");
				$url = $this->GetEditUrl(EW_TABLE_SHOW_DETAIL . "=class");
				...
				$DetailEditTblVar .= "class";
			}
			if ($GLOBALS["class_grid"]->DetailAdd && $Security->IsLoggedIn() && $Security->IsLoggedIn()) {
				$caption = $Language->Phrase("MasterDetailCopyLink");
				$url = $this->GetCopyUrl(EW_TABLE_SHOW_DETAIL . "=class");
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" class=\"ewMultiSelect\" value=\"" . ew_HtmlEncode($this->subject_id->CurrentValue) . "\" onclick=\"ew_ClickMultiCheckbox(event);\">";
	...
	function SetupOtherOptions() {
		...
		$DetailTableLink = "";
		$item = &$option->Add("detailadd_class");
		$url = $this->GetAddUrl(EW_TABLE_SHOW_DETAIL . "=class");
		$caption = $Language->Phrase("Add") . "&nbsp;" . $this->TableCaption() . "/" . $GLOBALS["class"]->TableCaption();
		$item->Body = "<a class=\"ewDetailAddGroup ewDetailAdd\" title=\"" . ew_HtmlTitle($caption) . "\" data-caption=\"" . ew_HtmlTitle($caption) . "\" href=\"" . ew_HtmlEncode($url) . "\">" . $caption . "</a>";
		$item->Visible = ($GLOBALS["class"]->DetailAdd && $Security->IsLoggedIn() && $Security->IsLoggedIn());
		if ($item->Visible) {
			if ($DetailTableLink <> "") $DetailTableLink .= ",";
			$DetailTableLink .= "class";
		}
...
<?php if (($subject->Export == "") || (EW_EXPORT_MASTER_RECORD && $subject->Export == "print")) { ?>
<?php
if ($subject_list->DbMasterFilter <> "" && $subject->getCurrentMasterTable() == "teacher") {
	if ($subject_list->MasterRecordExists) {
?>
<?php include_once "teachermaster.php" ?>
<?php
	}
}
?>
...
<?php } ?>
<input type="hidden" name="t" value="subject">
<?php if ($subject->getCurrentMasterTable() == "teacher" && $subject->CurrentAction <> "") { ?>
...
<?php if ($subject->class_id->Visible) { // class_id ?>
	...
	<?php } else { ?>
		<th data-name="class_id" class="<?php echo $subject->class_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $subject->SortUrl($subject->class_id) ?>',1);"><div id="elh_subject_class_id" class="subject_class_id">
...
<?php if ($subject->teacher_id->Visible) { // teacher_id ?>
	...
	<?php } else { ?>
		<th data-name="teacher_id" class="<?php echo $subject->teacher_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $subject->SortUrl($subject->teacher_id) ?>',1);"><div id="elh_subject_teacher_id" class="subject_teacher_id">
...
<span id="el<?php echo $subject_list->RowCnt ?>_subject_class_id" class="subject_class_id">
...
<span id="el<?php echo $subject_list->RowCnt ?>_subject_teacher_id" class="subject_teacher_id">
...
<?php
	...
	if ($subject->CurrentAction <> "gridadd")
		$subject_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
