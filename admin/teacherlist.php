...
class cteacher_list extends cteacher {
	...
	function __construct() {
		...
		$this->AddUrl = "teacheradd.php?" . EW_TABLE_SHOW_DETAIL . "=";
	...
	function GetFilterList() {
		...
		$sFilterList = ew_Concat($sFilterList, $this->pinterest->AdvancedSearch->ToJson(), ","); // Field pinterest
		if ($this->BasicSearch->Keyword <> "") {
	...
	function RestoreFilterList() {
		...
		// Field pinterest
		$this->pinterest->AdvancedSearch->SearchValue = @$filter["x_pinterest"];
		$this->pinterest->AdvancedSearch->SearchOperator = @$filter["z_pinterest"];
		$this->pinterest->AdvancedSearch->SearchCondition = @$filter["v_pinterest"];
		$this->pinterest->AdvancedSearch->SearchValue2 = @$filter["y_pinterest"];
		$this->pinterest->AdvancedSearch->SearchOperator2 = @$filter["w_pinterest"];
		$this->pinterest->AdvancedSearch->Save();
	...
	function BasicSearchSQL($arKeywords, $type) {
		...
		$this->BuildBasicSearchSQL($sWhere, $this->pinterest, $arKeywords, $type);
		return $sWhere;
	}
	...
	function SetupListOptions() {
		...
		// "detail_subject"
		$item = &$this->ListOptions->Add("detail_subject");
		$item->CssClass = "text-nowrap";
	...
	function RenderListOptions() {
		...
		if ($Security->IsLoggedIn()) {
			if (ew_IsMobile())
				...
				$oListOpt->Body = "<a class=\"ewRowLink ewView\" title=\"" . $viewcaption . "\" data-table=\"teacher\" data-caption=\"" . $viewcaption . "\" href=\"javascript:void(0);\" onclick=\"ew_ModalDialogShow({lnk:this,url:'" . ew_HtmlEncode($this->ViewUrl) . "',btn:null});\">" . $Language->Phrase("ViewLink") . "</a>";
		} else {
			$oListOpt->Body = "";
		}
		...
		if ($Security->IsLoggedIn()) {
			if (ew_IsMobile())
				...
				$oListOpt->Body = "<a class=\"ewRowLink ewEdit\" title=\"" . $editcaption . "\" data-table=\"teacher\" data-caption=\"" . $editcaption . "\" href=\"javascript:void(0);\" onclick=\"ew_ModalDialogShow({lnk:this,btn:'SaveBtn',url:'" . ew_HtmlEncode($this->EditUrl) . "'});\">" . $Language->Phrase("EditLink") . "</a>";
		} else {
			$oListOpt->Body = "";
		}
		...
		if ($Security->IsLoggedIn()) {
			if (ew_IsMobile())
				...
				$oListOpt->Body = "<a class=\"ewRowLink ewCopy\" title=\"" . $copycaption . "\" data-table=\"teacher\" data-caption=\"" . $copycaption . "\" href=\"javascript:void(0);\" onclick=\"ew_ModalDialogShow({lnk:this,btn:'AddBtn',url:'" . ew_HtmlEncode($this->CopyUrl) . "'});\">" . $Language->Phrase("CopyLink") . "</a>";
		} else {
			$oListOpt->Body = "";
		}
		...
		// "detail_subject"
		$oListOpt = &$this->ListOptions->Items["detail_subject"];
		if ($Security->IsLoggedIn()) {
			$body = $Language->Phrase("DetailLink") . $Language->TablePhrase("subject", "TblCaption");
			...
			$links = "";
			if ($GLOBALS["subject_grid"]->DetailView && $Security->IsLoggedIn() && $Security->IsLoggedIn()) {
				$caption = $Language->Phrase("MasterDetailViewLink");
				$url = $this->GetViewUrl(EW_TABLE_SHOW_DETAIL . "=subject");
			...
			if ($GLOBALS["subject_grid"]->DetailEdit && $Security->IsLoggedIn() && $Security->IsLoggedIn()) {
				$caption = $Language->Phrase("MasterDetailEditLink");
				$url = $this->GetEditUrl(EW_TABLE_SHOW_DETAIL . "=subject");
			...
			if ($GLOBALS["subject_grid"]->DetailAdd && $Security->IsLoggedIn() && $Security->IsLoggedIn()) {
				$caption = $Language->Phrase("MasterDetailCopyLink");
				$url = $this->GetCopyUrl(EW_TABLE_SHOW_DETAIL . "=subject");
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" class=\"ewMultiSelect\" value=\"" . ew_HtmlEncode($this->teacher_id->CurrentValue) . "\" onclick=\"ew_ClickMultiCheckbox(event);\">";
	...
	function SetupOtherOptions() {
		...
		if (ew_IsMobile())
			...
			$item->Body = "<a class=\"ewAddEdit ewAdd\" title=\"" . $addcaption . "\" data-table=\"teacher\" data-caption=\"" . $addcaption . "\" href=\"javascript:void(0);\" onclick=\"ew_ModalDialogShow({lnk:this,btn:'AddBtn',url:'" . ew_HtmlEncode($this->AddUrl) . "'});\">" . $Language->Phrase("AddLink") . "</a>";
		$item->Visible = ($this->AddUrl <> "" && $Security->IsLoggedIn());
		$option = $options["detail"];
		$DetailTableLink = "";
		$item = &$option->Add("detailadd_subject");
		$url = $this->GetAddUrl(EW_TABLE_SHOW_DETAIL . "=subject");
		$caption = $Language->Phrase("Add") . "&nbsp;" . $this->TableCaption() . "/" . $GLOBALS["subject"]->TableCaption();
		$item->Body = "<a class=\"ewDetailAddGroup ewDetailAdd\" title=\"" . ew_HtmlTitle($caption) . "\" data-caption=\"" . ew_HtmlTitle($caption) . "\" href=\"" . ew_HtmlEncode($url) . "\">" . $caption . "</a>";
		$item->Visible = ($GLOBALS["subject"]->DetailAdd && $Security->IsLoggedIn() && $Security->IsLoggedIn());
		if ($item->Visible) {
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fteacherlistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
	...
	function RenderRow() {
		...
		$this->pinterest->ViewCustomAttributes = "";
			...
			if ($this->photo->UseColorbox) {
				...
				$this->photo->LinkAttrs["data-rel"] = "teacher_x" . $this->RowCnt . "_photo";
...
fteacherlist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fteacherlist.Lists["x_sex"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fteacherlist.Lists["x_sex"].Options = <?php echo json_encode($teacher_list->sex->Options()) ?>;
fteacherlist.Lists["x_active"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fteacherlist.Lists["x_active"].Options = <?php echo json_encode($teacher_list->active->Options()) ?>;
...
<?php
	...
	if ($bSelectLimit) {
		if ($teacher_list->TotalRecs <= 0)
			$teacher_list->TotalRecs = $teacher->ListRecordCount();
	} else {
...
<form name="fteacherlistsrch" id="fteacherlistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($teacher_list->SearchWhere <> "") ? " in" : " in"; ?>
...
<div class="box ewBox ewGrid<?php if ($teacher_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> teacher">
...
<?php } ?>
<input type="hidden" name="t" value="teacher">
<div id="gmp_teacher" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
...
<?php if ($teacher->teacher_id->Visible) { // teacher_id ?>
	<?php if ($teacher->SortUrl($teacher->teacher_id) == "") { ?>
		<th data-name="teacher_id" class="<?php echo $teacher->teacher_id->HeaderCellClass() ?>"><div id="elh_teacher_teacher_id" class="teacher_teacher_id"><div class="ewTableHeaderCaption"><?php echo $teacher->teacher_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="teacher_id" class="<?php echo $teacher->teacher_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $teacher->SortUrl($teacher->teacher_id) ?>',1);"><div id="elh_teacher_teacher_id" class="teacher_teacher_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $teacher->teacher_id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($teacher->teacher_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($teacher->teacher_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($teacher->name->Visible) { // name ?>
	<?php if ($teacher->SortUrl($teacher->name) == "") { ?>
		<th data-name="name" class="<?php echo $teacher->name->HeaderCellClass() ?>"><div id="elh_teacher_name" class="teacher_name"><div class="ewTableHeaderCaption"><?php echo $teacher->name->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="name" class="<?php echo $teacher->name->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $teacher->SortUrl($teacher->name) ?>',1);"><div id="elh_teacher_name" class="teacher_name">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $teacher->name->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($teacher->name->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($teacher->name->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($teacher->birthday->Visible) { // birthday ?>
	<?php if ($teacher->SortUrl($teacher->birthday) == "") { ?>
		<th data-name="birthday" class="<?php echo $teacher->birthday->HeaderCellClass() ?>"><div id="elh_teacher_birthday" class="teacher_birthday"><div class="ewTableHeaderCaption"><?php echo $teacher->birthday->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="birthday" class="<?php echo $teacher->birthday->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $teacher->SortUrl($teacher->birthday) ?>',1);"><div id="elh_teacher_birthday" class="teacher_birthday">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $teacher->birthday->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($teacher->birthday->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($teacher->birthday->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($teacher->sex->Visible) { // sex ?>
	<?php if ($teacher->SortUrl($teacher->sex) == "") { ?>
		<th data-name="sex" class="<?php echo $teacher->sex->HeaderCellClass() ?>"><div id="elh_teacher_sex" class="teacher_sex"><div class="ewTableHeaderCaption"><?php echo $teacher->sex->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="sex" class="<?php echo $teacher->sex->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $teacher->SortUrl($teacher->sex) ?>',1);"><div id="elh_teacher_sex" class="teacher_sex">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $teacher->sex->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($teacher->sex->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($teacher->sex->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($teacher->phone->Visible) { // phone ?>
	<?php if ($teacher->SortUrl($teacher->phone) == "") { ?>
		<th data-name="phone" class="<?php echo $teacher->phone->HeaderCellClass() ?>"><div id="elh_teacher_phone" class="teacher_phone"><div class="ewTableHeaderCaption"><?php echo $teacher->phone->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="phone" class="<?php echo $teacher->phone->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $teacher->SortUrl($teacher->phone) ?>',1);"><div id="elh_teacher_phone" class="teacher_phone">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $teacher->phone->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($teacher->phone->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($teacher->phone->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($teacher->_email->Visible) { // email ?>
	<?php if ($teacher->SortUrl($teacher->_email) == "") { ?>
		<th data-name="_email" class="<?php echo $teacher->_email->HeaderCellClass() ?>"><div id="elh_teacher__email" class="teacher__email"><div class="ewTableHeaderCaption"><?php echo $teacher->_email->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="_email" class="<?php echo $teacher->_email->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $teacher->SortUrl($teacher->_email) ?>',1);"><div id="elh_teacher__email" class="teacher__email">
...
<?php if ($teacher->photo->Visible) { // photo ?>
	<?php if ($teacher->SortUrl($teacher->photo) == "") { ?>
		<th data-name="photo" class="<?php echo $teacher->photo->HeaderCellClass() ?>"><div id="elh_teacher_photo" class="teacher_photo"><div class="ewTableHeaderCaption"><?php echo $teacher->photo->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="photo" class="<?php echo $teacher->photo->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $teacher->SortUrl($teacher->photo) ?>',1);"><div id="elh_teacher_photo" class="teacher_photo">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $teacher->photo->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($teacher->photo->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($teacher->photo->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($teacher->active->Visible) { // active ?>
	<?php if ($teacher->SortUrl($teacher->active) == "") { ?>
		<th data-name="active" class="<?php echo $teacher->active->HeaderCellClass() ?>"><div id="elh_teacher_active" class="teacher_active"><div class="ewTableHeaderCaption"><?php echo $teacher->active->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="active" class="<?php echo $teacher->active->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $teacher->SortUrl($teacher->active) ?>',1);"><div id="elh_teacher_active" class="teacher_active">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $teacher->active->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($teacher->active->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($teacher->active->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php
...
<span id="el<?php echo $teacher_list->RowCnt ?>_teacher_teacher_id" class="teacher_teacher_id">
...
<span id="el<?php echo $teacher_list->RowCnt ?>_teacher_name" class="teacher_name">
...
<span id="el<?php echo $teacher_list->RowCnt ?>_teacher_birthday" class="teacher_birthday">
...
<span id="el<?php echo $teacher_list->RowCnt ?>_teacher_sex" class="teacher_sex">
...
<span id="el<?php echo $teacher_list->RowCnt ?>_teacher_phone" class="teacher_phone">
...
<span id="el<?php echo $teacher_list->RowCnt ?>_teacher_photo" class="teacher_photo">
<span>
...
<span id="el<?php echo $teacher_list->RowCnt ?>_teacher_active" class="teacher_active">
...
<?php if (!isset($teacher_list->Pager)) $teacher_list->Pager = new cPrevNextPager($teacher_list->StartRec, $teacher_list->DisplayRecs, $teacher_list->TotalRecs, $teacher_list->AutoHidePager) ?>
...
<?php } ?>
<?php if ($teacher_list->Pager->RecordCount > 0) { ?>
