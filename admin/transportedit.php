...
$transport_edit = NULL; // Initialize page object first

class ctransport_edit extends ctransport {
	...
	// Page object name
	var $PageObjName = 'transport_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			...
			// Load key from Form
			if ($objForm->HasValue("x_transport_id")) {
			...
			if (isset($_GET["transport_id"])) {
				$this->transport_id->setQueryStringValue($_GET["transport_id"]);
				$loadByQuery = TRUE;
			} else {
		...
		switch ($this->CurrentAction) {
			case "I": // Get a record to display
				if (!$loaded) { // Load record based on key
					...
					$this->Page_Terminate("transportlist.php"); // No matching record, return to list
				}
				break;
			Case "U": // Update
				$sReturnUrl = $this->getReturnUrl();
				if (ew_GetPageName($sReturnUrl) == "transportlist.php")
	...
	function LoadFormValues() {
		...
		if (!$this->route_name->FldIsDetailKey) {
			$this->route_name->setFormValue($objForm->GetValue("x_route_name"));
		}
		if (!$this->number_of_vehicle->FldIsDetailKey) {
			$this->number_of_vehicle->setFormValue($objForm->GetValue("x_number_of_vehicle"));
		}
		if (!$this->description->FldIsDetailKey) {
			$this->description->setFormValue($objForm->GetValue("x_description"));
		}
		if (!$this->route_fare->FldIsDetailKey) {
			$this->route_fare->setFormValue($objForm->GetValue("x_route_fare"));
		}
	}
	...
	function RestoreFormValues() {
		...
		$this->number_of_vehicle->CurrentValue = $this->number_of_vehicle->FormValue;
		$this->description->CurrentValue = $this->description->FormValue;
		$this->route_fare->CurrentValue = $this->route_fare->FormValue;
	}
	...
	function RenderRow() {
		...
		$this->route_fare->ViewCustomAttributes = "";
			...
			$this->transport_id->EditValue = $this->transport_id->CurrentValue;
			$this->transport_id->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// route_name
			$this->route_name->SetDbValueDef($rsnew, $this->route_name->CurrentValue, "", $this->route_name->ReadOnly);

			// number_of_vehicle
			$this->number_of_vehicle->SetDbValueDef($rsnew, $this->number_of_vehicle->CurrentValue, "", $this->number_of_vehicle->ReadOnly);

			// description
			$this->description->SetDbValueDef($rsnew, $this->description->CurrentValue, "", $this->description->ReadOnly);

			// route_fare
			$this->route_fare->SetDbValueDef($rsnew, $this->route_fare->CurrentValue, "", $this->route_fare->ReadOnly);
...
// Create page object
if (!isset($transport_edit)) $transport_edit = new ctransport_edit();

// Page init
$transport_edit->Page_Init();

// Page main
$transport_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$transport_edit->Page_Render();
?>
...
var CurrentForm = ftransportedit = new ew_Form("ftransportedit", "edit");

// Validate form
ftransportedit.Validate = function() {
...
ftransportedit.Form_CustomValidate = 
...
ftransportedit.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php $transport_edit->ShowPageHeader(); ?>
<?php
$transport_edit->ShowMessage();
?>
<form name="ftransportedit" id="ftransportedit" class="<?php echo $transport_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($transport_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $transport_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="transport">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<input type="hidden" name="modal" value="<?php echo intval($transport_edit->IsModal) ?>">
<div class="ewEditDiv"><!-- page* -->
<?php if ($transport->transport_id->Visible) { // transport_id ?>
	<div id="r_transport_id" class="form-group">
		<label id="elh_transport_transport_id" class="<?php echo $transport_edit->LeftColumnClass ?>"><?php echo $transport->transport_id->FldCaption() ?></label>
		<div class="<?php echo $transport_edit->RightColumnClass ?>"><div<?php echo $transport->transport_id->CellAttributes() ?>>
<span id="el_transport_transport_id">
<span<?php echo $transport->transport_id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $transport->transport_id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="transport" data-field="x_transport_id" name="x_transport_id" id="x_transport_id" value="<?php echo ew_HtmlEncode($transport->transport_id->CurrentValue) ?>">
<?php echo $transport->transport_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($transport->route_name->Visible) { // route_name ?>
	<div id="r_route_name" class="form-group">
		<label id="elh_transport_route_name" for="x_route_name" class="<?php echo $transport_edit->LeftColumnClass ?>"><?php echo $transport->route_name->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $transport_edit->RightColumnClass ?>"><div<?php echo $transport->route_name->CellAttributes() ?>>
<span id="el_transport_route_name">
...
<?php echo $transport->route_name->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($transport->number_of_vehicle->Visible) { // number_of_vehicle ?>
	<div id="r_number_of_vehicle" class="form-group">
		<label id="elh_transport_number_of_vehicle" for="x_number_of_vehicle" class="<?php echo $transport_edit->LeftColumnClass ?>"><?php echo $transport->number_of_vehicle->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $transport_edit->RightColumnClass ?>"><div<?php echo $transport->number_of_vehicle->CellAttributes() ?>>
<span id="el_transport_number_of_vehicle">
...
<?php echo $transport->number_of_vehicle->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($transport->description->Visible) { // description ?>
	<div id="r_description" class="form-group">
		<label id="elh_transport_description" for="x_description" class="<?php echo $transport_edit->LeftColumnClass ?>"><?php echo $transport->description->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $transport_edit->RightColumnClass ?>"><div<?php echo $transport->description->CellAttributes() ?>>
<span id="el_transport_description">
...
<?php echo $transport->description->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($transport->route_fare->Visible) { // route_fare ?>
	<div id="r_route_fare" class="form-group">
		<label id="elh_transport_route_fare" for="x_route_fare" class="<?php echo $transport_edit->LeftColumnClass ?>"><?php echo $transport->route_fare->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $transport_edit->RightColumnClass ?>"><div<?php echo $transport->route_fare->CellAttributes() ?>>
<span id="el_transport_route_fare">
...
<?php echo $transport->route_fare->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
<?php if (!$transport_edit->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $transport_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $transport_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
